...
`.spec.interval` is an optional field that specifies the interval at which the
ImagePolicy must refresh the digest of the latest tag when the reflection policy is set
to `Always`. The value must be in a
[Go recognized duration string format](https://pkg.go.dev/time#ParseDuration),
e.g. `10m0s` to reconcile the object every 10 minutes.
The default value is `10m0s` (10 minutes).
