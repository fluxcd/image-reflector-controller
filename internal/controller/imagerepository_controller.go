...
func notify(r events.EventRecorder, oldObj, newObj conditions.Setter, nextScanMsg string) {
	...
	if conditions.IsReady(oldObj) && conditions.IsReady(newObj) &&
		...
		r.Eventf(newObj, oldObj, corev1.EventTypeNormal, ready.Reason, eventv1.ActionReconciled, "%s", ready.Message)
		return
	}
	...
	if !conditions.IsReady(newObj) {
		r.Eventf(newObj, oldObj, corev1.EventTypeWarning, ready.Reason, eventv1.ActionFailed, "%s", ready.Message)
		return
	}
	r.Eventf(newObj, oldObj, eventv1.EventTypeTrace, meta.SucceededReason, eventv1.ActionReconciled, "%s", nextScanMsg)
}
